...
export const resolvers = {
  ...
  Mutation: {
    deleteGame(parent, args, context) {
      return _database.games.filter((item) => item.id !== args.id);
    },
    ...
    addGame(parent, args, context) {
      ...
      _database.games = [..._database.games, gameToAdd];
      return gameToAdd;
    },

    updateGame(parent, args, context) {
      _database.games = _database.games.map((item) => {
        if (item.id === args.id) {
          return { ...item, ...args.updatingData };
        }
        return item;
      });
